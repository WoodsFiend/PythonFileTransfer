...
import socket, sys, os, subprocess, re
...
if ready == "READY":
    ...
    if(data1 == 'OK' and command == 'GET'):
        ...
        #print()
        print("client receiving file " + fileName + " ("+ str(numBytes)+" bytes)")
        ...
        try:
            ...
            print()
            print("  CLIENT ERROR - " + fileName + " could not be saved on client, Permission Denied")
            print()
            print()
            sys.exit()
        ...
        while total < numBytes:
            if (numBytes-total) == numBytes%1024:
               amtRcv = numBytes%1024
            data = s.recv(amtRcv)
            total += len(data)
